<?php
	...
	class woocommerce_feedoptimise_feed
	{
	    ...
	    function render()
	    {
            //fix for websites that gzip the response but do not send the gzip header
            if(isset($_GET['woocommerce_fo_encoding']) && $_GET['woocommerce_fo_encoding'] != '')
            {
                header("Content-Encoding: ".$_GET['woocommerce_fo_encoding']);
            }
	    ...
	    protected function _render_product_feed() {
			...
			if(isset($_GET['woocommerce_fo_enable_php_errors']))
			{
			...
			$skip_hidden 		= (int)(isset($_GET['woocommerce_fo_skip_hidden']) ? $_GET['woocommerce_fo_skip_hidden'] : 1);
			$skip_having_parent = (int)(isset($_GET['woocommerce_fo_skip_having_parent']) ? $_GET['woocommerce_fo_skip_having_parent'] : 1);
	        ...
	        while(count($products))
            {
	            foreach($products as $post)
	            {
	                ...
	                if($skip_hidden && $woocommerce_product->visibility == 'hidden')
	                {
						continue;
					}

	                if($skip_having_parent && $woocommerce_product->get_parent())
	                {
	                	continue;
	                }
	                ...
	                if ( $woocommerce_product->has_child() ) {
	                    ...
	                    foreach ( $children as $child ) {
	                        ...
	                        $variant = $this->_getFeedItem($child_product, true);
	    ...
	    protected function _getFeedItem($woocommerce_product, $is_child = false)
	    {
	        ...
	        if($feed_item->type!='variation')
	        {
	        	...
	        	if(get_the_excerpt($woocommerce_product->id)!=get_the_content($woocommerce_product->id))
	        	{
	        		$feed_item->short_description 	= get_the_excerpt($woocommerce_product->id);
	        	}
	        	
	        	$feed_item->product_url 		= get_permalink($woocommerce_product->id);
	        }

	        //Fix for variant images
	        if($is_child === true)
	        {
	        	if(function_exists('wc_get_product_attachment_props'))
	        	{
					$variant_image = wc_get_product_attachment_props($woocommerce_product->get_image_id());
					$feed_item->image_url_variant_original = $variant_image['full_src'];
					$feed_item->image_url_variant_thumb    = $variant_image['thumb_src'];
	        	}
	        }
	        ...
	        if(method_exists($woocommerce_product,'get_stock_quantity'))
	        {
                $feed_item->stock_quantity = $woocommerce_product->get_stock_quantity();
            }
