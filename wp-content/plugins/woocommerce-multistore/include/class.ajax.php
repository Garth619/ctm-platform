...
class WOO_MSTORE_ajax {
	function __construct() {
		...
		//bulk distribution
		add_action( 'wp_ajax_woosl-bulk-distribution-save', array( $this, 'bulk_distribution_save' ) );
	}
...
//		restore_current_blog();
	...
	function bulk_distribution_save() {
		if ( empty( $_REQUEST['woocommerce_multisite_bulk_distribution_edit_nonce'] ) ) {
			die();
		}

		$list_blogID_postID = (array) $_POST['ids'];

		global $WOO_MSTORE;

		$response = array( 'errors' => array() );

		foreach ( $list_blogID_postID as $item ) {
			list( $post_blog_id, $post_id ) = explode( "_", $item );

			$product = wc_get_product( $post_id );

			if ( $product instanceof WC_Product ) {
				if ( $WOO_MSTORE->product_interface->is_slave_product( $product ) ) {
					$response['errors'][] = sprintf(
						__( "Product %d is child, can't be replicated. Ignoring.", 'woonet' ),
						$post_id
					);
				} else {
					$WOO_MSTORE->product_interface->process_product( $product->get_id() );
				}
			} else {
				$this->log( $item, __LINE__, 'error' );

				$response['errors'][] = sprintf(
					__( "Product %s was not found, can't be replicated. Ignoring.", 'woonet' ),
					$post_id
				);
			}
		}
