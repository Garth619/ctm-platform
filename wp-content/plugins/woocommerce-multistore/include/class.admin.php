...
class WOO_MSTORE_admin {
	...
	function wp_enqueue_woocommerce_style() {
		...
		if (
			...
			(
				...
				&&
				'woocommerce_page_woonet-woocommerce-products-network' !== $screen->id
			)
		) {
			return;
		}

		wp_enqueue_script( 'quick-edit', WOO_MSTORE_URL . '/assets/js/quick-edit.js' );
		wp_localize_script( 'quick-edit', 'woonet_options', $this->functions->get_options() );
	}
	...
	public function quick_edit( $column_name, $post_type ) {
		...
		echo '<input type="hidden" name="_is_master_product" value="" />';

		include( WOO_MSTORE_PATH . '/include/admin/views/html-quick-edit-product.php' );
		include( WOO_MSTORE_PATH . '/include/admin/views/html-quick-edit-slave-product.php' );
	}
	...
	public function add_quick_edit_inline_data( $post ) {
		...
		echo '<div class="hidden" id="woocommerce_multistore_inline_' . absint( $post->ID ) . '">';
			...
			if ( empty( get_post_meta( $post->ID, '_woonet_network_is_child_product_id', true ) ) ) {
				...
				printf(
					'<div class="product_blog_id">%s</div>
