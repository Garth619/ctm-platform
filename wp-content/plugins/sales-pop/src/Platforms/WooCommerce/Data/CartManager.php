...
use BeeketingConnect_beeketing_woocommerce_salespop\Common\Data\Manager\CartManagerAbstract;
use BeeketingConnect_beeketing_woocommerce_salespop\Common\Data\Model\Cart;
use BeeketingConnect_beeketing_woocommerce_salespop\Common\Data\Model\CartItem;
use BeeketingConnect_beeketing_woocommerce_salespop\Platforms\WooCommerce\Helper;
use BeeketingConnect_beeketing_woocommerce_salespop\Platforms\WooCommerce\Models\WoocommerceCart;
...
class CartManager extends CartManagerAbstract
{
    ...
    public function get($setCookie = true)
    {
        ...
        $idKey = md5($cartToken . $cart->subtotal . $cart->cart_total . $cart->total . $itemsKey);

        // Return cart from cookie
        if (isset($_COOKIE[CommonConstants::COOKIE_BEEKETING_CART_DATA])) {
            $cartData = json_decode(stripslashes($_COOKIE[CommonConstants::COOKIE_BEEKETING_CART_DATA]), true);
            if (isset($cartData[$idKey])) {
                return $cartData[$idKey];
            }
        }
        ...
        // Set cart cookie
        if ($setCookie) {
            $cookie_data[$idKey] = $result;
            setcookie(CommonConstants::COOKIE_BEEKETING_CART_DATA, json_encode($cookie_data), time() + 300, '/');
        }
