...
if(!is_numeric($btn_font_size)) {
    $btn_font_size = preg_replace( '/[^0-9]/', "", $btn_font_size );
}
if (!is_numeric($btn_line_height)){
    $btn_line_height = preg_replace( '/[^0-9]/', "", $btn_line_height );
}
if ( $btn_font_size ) {
    $btn_style_inline .= 'font-size:'. esc_attr( $btn_font_size ) .'px;';
}
if ( $btn_line_height && ( $btn_size != 'porto-btn-custom' || !$btn_height ) ) {
    $btn_style_inline .= 'line-height:'. esc_attr( $btn_line_height ) .'px;';
}

$style .= $btn_style_inline;
if ( $btn_size == 'porto-btn-custom' ){
    if ( $btn_width ) {
...
$output .= '<a class="porto-btn '.esc_attr( $is_vc_49_plus ).' '.esc_attr($btn_size).' '.esc_attr($btn_hover).esc_attr($el_class).'"'. $link_attrs .' data-hover="'.esc_attr($btn_title_color_hover).'" data-border-color="'.esc_attr($btn_color_border).'" data-bg="'.esc_attr($btn_bg_color).'" data-hover-bg="'.esc_attr($btn_bg_color_hover).'" data-border-hover="'.esc_attr($btn_color_border_hover).'" data-shadow-click="'.esc_attr($shadow_click).'" data-shadow="'.esc_attr($box_shadow).'" style="'.esc_attr($style).'">';
...
$wrapper_class = '';
switch ($btn_align) {
        case 'porto-btn-inline':     $wrapper_class = 'porto-btn-ctn-inline';     break;
        case 'porto-btn-center':     $wrapper_class = 'porto-btn-ctn-center';     break;
        case 'porto-btn-right':      $wrapper_class = 'porto-btn-ctn-right';      break;
        case 'porto-btn-left':
        default:                $wrapper_class = 'porto-btn-ctn-left';       break;
