...
if ( ! class_exists( 'Porto_Speed_Optimize_Wizard' ) ) {
    ...
    class Porto_Speed_Optimize_Wizard {
        ...
        public function speed_optimize_wizard() {
            ...
            wp_localize_script( 'porto-speed-optimize', 'porto_speed_optimize_wizard_params', array(
        ...
        ?>
            ...
            <head>
                ...
                <title><?php esc_html_e( 'Theme &rsaquo; Speed Optimize Wizard', 'porto' ); ?></title>
        ...
        public function porto_speed_optimize_wizard_welcome() {
            ?>
            <h1><?php printf( esc_html__( 'Welcome to the Speed Optimize Wizard for %s.' ), wp_get_theme()); ?></h1>
        ...
        public function porto_speed_optimize_wizard_revslider() {
            ...
            <?php if ( !empty( $rev_pages ) ) : ?>
                <p style="margin-bottom: 5px;"><?php esc_html_e( 'Only below pages are using revolution slider feature.', 'porto' ); ?></p>
                <ul>
                <?php
                foreach( $rev_pages as $page_id ) {
                    $page = get_post( $page_id );
                    if ( $page ) {
                        echo '<li>'.$page->post_type . ': <a href="'. esc_url( get_permalink( $page_id ) ) .'" target="_blank">'. $page->post_title .'</a></li>';
                    }
                }
                ?>
                </ul>
        ...
        public function porto_speed_optimize_wizard_revslider_save() {
            ...
            global $porto_settings_optimize;
            if ( isset( $_POST['optimize_revslider'] ) && 'true' == $_POST['optimize_revslider'] ) {
                $porto_settings_optimize['optimize_revslider'] = true;
                $revsliderPages = porto_get_used_shortcode_list( array( 'rev_slider', 'rev_slider_vc' ), true );
                if ( $revsliderPages && !empty( $revsliderPages ) ) {
                    $porto_settings_optimize['optimize_revslider_pages'] = $revsliderPages;
                }
            } else {
        ...
        private function _get_plugins() {
            ...
            foreach ( $instance->plugins as $slug => $plugin ) {
                if ( !isset( $plugin['visibility'] ) || 'speed_wizard' != $plugin['visibility'] || $instance->is_plugin_active( $slug ) && false === $instance->does_plugin_have_update( $slug ) ) {
                    continue;
                } else {
