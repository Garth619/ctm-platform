        ...
        if ( class_exists('WCML_Multi_Currency') ) {
            ...
            foreach ($currencies as $currency) {
                if ($woocommerce_wpml->settings['currency_options'][$currency]['languages'][$sitepress->get_current_language()] == 1 ) {
                    ...
                    if ($selected) {
                        ...
                        $other_c .= '<li rel="' . $currency . '" class="menu-item"><a class="nolink" href="#">' . $currency_format . '</a></li>';
                    }
                }
            }
            ?>
            <ul id="menu-currency-switcher" class="currency-switcher porto-view-switcher mega-menu show-arrow">
                <li class="menu-item<?php if ($other_c) echo ' has-sub' ?> narrow">
                    <a class="nolink" href="#"><?php echo $active_c ?></a>
                    <?php if ($other_c) : ?>
                        <div class="popup">
                            <div class="inner">
            ...
            if (!empty($languages)) {
                ...
                <ul class="view-switcher porto-view-switcher mega-menu show-arrow">
                    <li class="menu-item<?php if ($other_langs) echo ' has-sub' ?> narrow">
                        <a class="nolink" href="#"><?php echo $active_lang ?></a>
                        <?php if ($other_langs) : ?>
                            <div class="popup">
                                <div class="inner">
                                    <ul class="sub-menu">
            ...
            if (!empty($languages)) {
                ...
                foreach ($languages as $language) {
                    if ($language != $q_config['language']) {
                        ...
                        $other_langs .= '<span class="flag"><img src="'.$flag_location.$q_config['flag'][$language].'" alt="" /></span>';
                        $other_langs .= $q_config['language_name'][$language];
                        $other_langs .= '</a></li>';
                    } else {
                        $active_lang .= '<span class="flag"><img src="'.$flag_location.$q_config['flag'][$language].'" alt="" /></span>';
...
function porto_header_side_menu() {
    ...
    if ( has_nav_menu( 'main_menu' ) || $main_menu ) {
        $args = array(
            'container' => '',
            'menu_class' => 'sidebar-menu' . ((has_nav_menu( 'sidebar_menu' ) || porto_get_meta_value('sidebar_menu')) ? ' has-side-menu' : ''),
            'before' => '',
            'after' => '',
            'link_before' => '',
            'link_after' => '',
    ...
    if ($output && $html) {
        ...
        $output = '<ul class="' . 'sidebar-menu' . ((has_nav_menu( 'sidebar_menu' ) || porto_get_meta_value('sidebar_menu')) ? ' has-side-menu' : '') . '" id="menu-main-menu">' . $html . '</ul>';
    }
...
function porto_search_form_content() {
    ...
    <form action="<?php echo home_url(); ?>/" method="get"
        ...
        <div class="searchform-fields">
            <span class="text"><input name="s" type="text" value="<?php echo get_search_query() ?>" placeholder="<?php echo $placeholder_text; ?>" autocomplete="off" /></span>
...
function porto_header_socials() {
    ...
    if ($porto_settings['header-social-whatsapp']) :
        ?><a <?php echo $nofollow ?> class="share-whatsapp" style="display:none" href="whatsapp://send?text=<?php echo esc_url($porto_settings['header-social-whatsapp']) ?>" data-action="share/whatsapp/share" title="<?php echo __('WhatsApp', 'porto') ?>"><?php echo __('WhatsApp', 'porto') ?></a><?php
    endif;
...
function porto_minicart() {
    ...
    if ( class_exists( 'WooCommerce' ) ) :
        ?>
        <div id="mini-cart" class="mini-cart <?php echo $minicart_type ?>">
            <div class="cart-head">
            <?php
                if ( 'minicart-inline' == $minicart_type ) {
                    ...
                    if (defined('WP_CACHE') && WP_CACHE) {
                        $_cartQty = $_cartTotal = '<i class="fa fa-spinner fa-pulse"></i>';
                    } else {
                        $_cartQty = $woocommerce->cart->cart_contents_count;
                        $_cartTotal = $woocommerce->cart->get_cart_subtotal();
                    }
                    printf( $format, $_cartQty, $_cartTotal );
                } else {
                    $format = '<i class="minicart-icon"></i><span class="cart-items">%s</span><span class="cart-items-text">%s</span>';
                    if (defined('WP_CACHE') && WP_CACHE) {
                        $_cartQty = $_cartQty1 = '<i class="fa fa-spinner fa-pulse"></i>';
                    } else {
                        $_cartQty = $woocommerce->cart->cart_contents_count;
                        $_cartQty1 = sprintf( _n( '%d item', '%d items', $_cartQty, 'porto' ), $_cartQty );
                    }
                    printf( $format, $_cartQty, $_cartQty1 );
                }
            ?>
            </div>
...
function porto_header_builder_layout() {
    ...
    if ( !isset( $porto_header_builder_layout ) ) {
        ...
        } else if ( !empty( $selected_layout ) && isset( $selected_layout['selected_layout'] ) && $selected_layout['selected_layout'] && isset( $header_layouts[$selected_layout['selected_layout']] ) ) {
...
function porto_get_minicart_type() {
    global $porto_settings;
    $header_type = porto_get_header_type();
    if ( ( (int)$header_type >= 1 && (int)$header_type <= 9 ) || 18 == $header_type || 19 == $header_type || empty( $header_type ) ) {
    ...
    foreach( $elements as $element ) {
        ...
        if ( is_array( $element ) ) {
            ...
            foreach( $element as $key => $value ) {
                ...
                } else if ( 'html' == $key && $value ) {
                    echo '<div class="custom-html">';
                        echo do_shortcode( $value );
                    echo '</div>';
                } else if ( 'logo' == $key ) {
                    echo porto_logo();
                } else if ( 'currency-switcher' == $key ) {
                    echo porto_currency_switcher();
                } else if ( 'language-switcher' == $key ) {
                    echo porto_view_switcher();
                } else if ( 'mini-cart' == $key ) {
                    echo porto_minicart();
                } else if ( 'contact' == $key ) {
