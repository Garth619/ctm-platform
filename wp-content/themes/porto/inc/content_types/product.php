...
function porto_product_meta_fields() {
    ...
    $product_fields = array(
        'product_layout' => array(
            ...
            'default' => 'theme',
            'options' => array(
                '' => __( 'Theme Options', 'porto' ),
                'default' => __( 'Default', 'porto' ),
                'extended' => __( 'Extended', 'porto' ),
                'full_width' => __( 'Full Width', 'porto' ),
                'grid' => __( 'Grid', 'porto' ),
                'sticky_info' => __( 'Sticky Info', 'porto' ),
                'sticky_both_info' => __( 'Sticky Left & Right Info', 'porto' ),
                'transparent' => __( 'Transparent Images', 'porto' ),
                'centered_vertical_zoom' => __( 'Centered Vertical Zoom', 'porto' ),
                'left_sidebar' => __( 'Left Sidebar', 'porto' ),
            )
        ),
        'product_custom_block' => array(
            'name' => 'product_custom_block',
            'title' => __('Custom Block', 'porto'),
            'desc' => __( 'Please input block slug name. This is used for Extended, Sticky Info and Wide Grid layout.', 'porto' ),
            'type' => 'text',
        ),
    ...
    $meta_fields = array_merge($product_fields, $custom_tabs, array(
        // Share
        'product_share' => array(
            'name' => 'product_share',
...
function porto_product_cat_meta_fields() {
    ...
    // View Mode
    $meta_fields = array_insert_after('view_mode', $meta_fields, 'filter_layout', array(
        'name' => 'filter_layout',
    ...
    // Columns
    $meta_fields = array_insert_after('filter_layout', $meta_fields, 'product_cols', array(
        'name' => 'product_cols',
...
if ( $attribute_taxonomies = wc_get_attribute_taxonomies() ) {
    foreach ( $attribute_taxonomies as $tax ) {
        if ( 'color' === $tax->attribute_type ) {
            add_action( wc_attribute_taxonomy_name( $tax->attribute_name ) . '_add_form_fields', 'porto_add_product_attribute_color_fields', 100, 1 );
            ...
            add_action( 'created_term', 'porto_save_product_attribute_color_values', 100, 3 );
        }
    }
}
