    ...
    class Porto_Live_Search {
        ...
        public function ajax_search() {
            ...
            foreach( $posts as $post ) {
                if ( class_exists( 'Woocommerce' ) && ( $post->post_type === 'product' || $post->post_type === 'product_variation' ) ) {
        ...
        private function search_posts( $args, $query, $post_type = array( 'post', 'page', 'portfolio', 'event' ) ) {
            ...
            return ( $search_funtion === 'get_posts' || !function_exists( $search_funtion ) ? get_posts( $args ) : $search_funtion( $search_query, $args ) );
        }
        ...
        private function search_add_category_args( $args ) {
            if ( isset( $_REQUEST['cat'] ) && $_REQUEST['cat'] && '0' != $_REQUEST['cat'] ) {
                ...
                } else if ( 'post' == $porto_settings['search-type'] ) {
                    $args['category'] = esc_attr( $_REQUEST['cat'] );
                } else if ( 'portfolio' == $porto_settings['search-type'] ) {
                    $args['tax_query'] = array();
                    $args['tax_query'][] = array(
