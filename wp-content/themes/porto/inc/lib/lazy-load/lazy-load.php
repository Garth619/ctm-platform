    ...
    static function add_image_placeholders( $content ) {
        ...
        foreach ( $matches[0] as $imgHTML ) {
            ...
            if ( ! preg_match( "/src=['\"]data:image/is", $imgHTML ) ) {
                ...
                $replaceHTML = preg_replace( '/<img(.*?)src=/is', '<img$1src="'. esc_url( $lazy_image ).'" data-original=', $imgHTML );
                $replaceHTML = preg_replace( '/<img(.*?)srcset=/is', '<img$1srcset="'. esc_url( $lazy_image ) .' 100w" data-srcset=', $replaceHTML );
