                        ...
                        if ( taxonomy_exists( $name ) ) {
                            ...
                            foreach ( $terms as $term ) {
                                if ( in_array( $term->slug, $selected_terms ) ) {
                                    $attribute_terms[] = array( 'id' => md5( $term->slug ), 'label' => $term->name, 'term_id' => $term->term_id );
                                }
                            }
                        } else {
                        ...
                        <div class="porto_swatches_section">
                            <p class="form-field-header">
                                <span class="swatch_label">
                                    <strong><a class="wcsap_edit_field row-title" href="javascript:void(0)"><?php echo $current_label; ?></a></strong>
                                </span>
                            </p>
                            <div class="form-field-body">
                                <p class="form-field">
                                    <label for="swatch_option_<?php echo $key_attr; ?>_type"><?php esc_html_e( 'Type', 'porto' ); ?></label>
                                    <select class="swatch_option_type" id="swatch_option_<?php echo $key_attr; ?>_type" name="swatch_options[<?php echo $key; ?>][type]"<?php echo $global_attribute ? ' disabled="disabled"' : ''; ?>>
                                ...
                                <p class="form-field swatch_field_image" style="<?php echo $current_type != 'image' ? 'display:none;' : ''; ?>">
                                    <label for="swatch_option_<?php echo $key_attr; ?>_size"><?php esc_html_e('Size', 'porto'); ?></label>
                                    <?php $image_sizes = get_intermediate_image_sizes(); ?>
                                    <select id="swatch_option_pa_color_size" name="swatch_options[<?php echo $key; ?>][size]">
                                                ...
                                                <tr>
                                                    <td class="attribute_swatch_preview">
                                                        <div class="select-option">
                                                            <a id="swatch_option_<?php echo $key_attr; ?>_<?php echo $attribute_term['id']; ?>_color_preview_image" href="javascript:void(0)" class="image swatch_field_image" style="<?php echo $current_type == 'image' ? '' : 'display:none;'; ?>">
                                                                <img src="<?php echo esc_url( $current_attribute_image_src ); ?>" class="wp-post-image" />
                                                            </a>
                                                            <a id="swatch_option_<?php echo $key_attr; ?>_<?php echo $attribute_term['id']; ?>_color_preview_swatch" href="javascript:void(0)" class="swatch swatch_field_color" style="background-color:<?php echo $current_attribute_color; ?>;<?php echo $current_type == 'color' ? '' : 'display:none;'; ?>"><?php echo $attribute_term['label']; ?>
                                                            </a>
                                                        </div>
                                                    </td>
                                                    <td class="attribute_swatch_label">
                                                        <strong><a class="wcsap_edit_field row-title" href="javascript:void(0)"><?php echo $attribute_term['label']; ?></a></strong>
                                                    </td>
                                                    <td class="attribute_swatch_input swatch_field_color" style="<?php echo $current_type == 'color' ? '' : 'display:none;'; ?>">
                                                        <div <?php if ( !$global_attribute_item ) : ?>class="porto-meta-color"<?php endif; ?>>
                                                            <input type="text" <?php if ( !$global_attribute_item ) : ?>name="swatch_options[<?php echo $key; ?>][attributes][<?php echo esc_attr( $attribute_term['id'] ); ?>][color]"<?php else: ?>disabled="disabled"<?php endif; ?> value="<?php echo $current_attribute_color; ?>" class="porto-color-field" />
                                                        </div>
                                                    </td>

                                                    <td class="attribute_swatch_input swatch_field_image" style="<?php echo $current_type == 'image' ? '' : 'display:none;'; ?>">

                                                        <div>
                                                            <div id="swatch_option_<?php echo $key_attr; ?>_<?php echo $attribute_term['id']; ?>_image_thumbnail" style="float:left;margin-top:3px;margin-right:10px;">
                                                                <img src="<?php echo $current_attribute_image_src; ?>" alt="<?php esc_html_e( 'Thumbnail Preview', 'porto' ); ?>" class="wp-post-image" width="16" height="16">
                                                            </div>
                                                            <input class="upload_image_id" type="hidden" id="swatch_option_<?php echo $key_attr; ?>_<?php echo $attribute_term['id']; ?>_image" name="swatch_options[<?php echo $key; ?>][attributes][<?php echo esc_attr( $attribute_term['id'] ); ?>][image]" value="<?php echo $current_attribute_image_id; ?>" />
                                                            <button type="submit" class="upload_swatch_image_button button" rel="<?php echo $post_id; ?>"><?php esc_html_e( 'Upload/Add image', 'porto' ); ?></button>
                                                            <button type="submit" class="remove_swatch_image_button button" rel="<?php echo $post_id; ?>"><?php esc_html_e( 'Remove image', 'porto' ); ?></button>
                                                        </div>
